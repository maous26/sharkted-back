"""
Collector KITH EU - Extraction via API Shopify JSON.
KITH utilise Shopify, donc accès direct à l'API /products.json
"""
import json
import re
...
def fetch_kith_product(url: str) -> DealItem:
    """Récupère et parse un produit KITH via API Shopify."""
            ...
            if resp.status_code == 200:
                ...
                if 'product' in data:
                    ...
                    return DealItem(
                        ...
                        currency="EUR",  # KITH EU est en euros
...
def discover_kith_products(limit: int = 50) -> List[str]:
    """Découvre les URLs de produits en soldes sur KITH via API Shopify."""
    scraper = cloudscraper.create_scraper()
    urls = []
    
    # Collections de soldes
    collections = ["footwear-sale", "apparel-sale"]
    
    for collection in collections:
        api_url = f"{BASE_URL}/collections/{collection}/products.json?limit={min(limit, 250)}"
        ...
        try:
            ...
            print(f"Error discovering kith products: {e}")
