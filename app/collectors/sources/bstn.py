"""
Collector BSTN - Extraction de sneakers premium.

BSTN (bstn.com) - Store Shopify-like, données depuis JSON-LD et meta tags.
"""
import re
import json
...
def _extract_sku_from_url(url: str) -> Optional[str]:
    """Extrait le SKU de l'URL BSTN."""
    # Format: /eu_en/product-name-123456.html
    match = re.search(r'-(\d+)\.html', url)
    if match:
        return match.group(1)
    # Alternative: /product-name.html
    match = re.search(r'/([^/]+)\.html$', url)
    return match.group(1) if match else None


def _extract_product_data(html: str, url: str) -> dict:
    """Extrait les données produit depuis le HTML BSTN."""
    data = {
        "name": None,
        "price": None,
        "original_price": None,
    ...
    # 2. Prix depuis meta tags
    if not data["price"]:
        price_meta = re.search(r'<meta property="product:price:amount"[^>]*content="([0-9.]+)"', html)
        if price_meta:
            data["price"] = float(price_meta.group(1))

    # 3. Prix original (barré)
    # Pattern: class="old-price"... ou data-price-type="oldPrice"
    old_price = re.search(r'(?:old-price|oldPrice|was-price)[^>]*>\s*[^0-9]*([0-9,.]+)', html, re.IGNORECASE)
    if old_price:
        try:
            price_str = old_price.group(1).replace(",", ".").replace(" ", "")
...
def fetch_bstn_product(url: str) -> DealItem:
    ...
    scraper, headers = create_stealth_scraper("bstn")
